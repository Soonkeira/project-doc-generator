...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-v1.3.0-blue.svg" alt="Version: v1.3.0">
...
5. **三种运行模式**：
   ...
   - **查询模式（默认只读）**：直接提问业务问题，例如"订单取消以后做了什么？"，Skill 只读取 `00-项目知识地图.md`、`00-源码索引.md` 与 BL 条目并作答，**不写任何文件**；条目缺失或可能过期时明确告知"以源码为准"，并建议运行同步模式
...
5. **Three operating modes**:
   ...
   - **Query mode (read-only by default)**: just ask a business question, e.g. "What happens after an order is cancelled?". The Skill reads only `00-Project Knowledge Map.md`, `00-Source Index.md` and the BL entries to answer and **writes no files**; when an entry is missing or possibly stale it states explicitly that "the source code prevails" and suggests running sync mode
...
| 🔁 源码反向索引 / Source-to-BL Reverse Index | 从源码反查业务逻辑：源码文件与符号 → BL、数据表/迁移 → BL、API/入口 → BL、配置项 → BL 四张反向索引表（`00-源码索引.md`）/ Looks business logic up from source code: four reverse index tables (`00-Source Index.md`) for source files and symbols → BL, tables/migrations → BL, APIs/entry points → BL, and configuration items → BL |
| 🔗 双向索引一致性 / Bidirectional Index Consistency | 源码索引由各 BL 条目的源码证据、数据变化、触发入口与外部影响汇总而来，必须与 BL 条目保持一致；不一致视为校验失败 / The source index is aggregated from each BL entry's source evidence, data changes, trigger entry points and external impacts and must stay consistent with the BL entries; any mismatch counts as a validation failure |
| ⏰ 文档过期检测 / Document Staleness Detection | 源码证据文件在基线之后被改动（**含未提交修改**）且未重新验证时，标记 `⚠ 可能过期` / Marks entries as `⚠ possibly stale` when their evidence files changed after the baseline — **including uncommitted changes** — without re-verification |
...
| 00b | 源码索引 / Source Index | 源码 → BL 反向索引：源码文件与符号、数据表/迁移、API/入口、配置项四张反向索引表，另含未覆盖的源码文件、覆盖范围与限制、维护规则 |
