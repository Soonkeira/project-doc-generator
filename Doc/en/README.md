> Version: v1.4.1 ｜ Author: [Soonkeira](https://github.com/Soonkeira)
...
5. **Re-verify only the affected entries**: the document set is not rewritten as a whole; unaffected entries stay as they are (no content rewrite, no renumbering, no change to an existing ID). Each BL entry also has its own "Last Verified Version" commit.
6. **Refresh the freshness status and the source index**: after the affected entries are re-verified their "Freshness Status" (one of four values) is updated, and changes to source evidence, data changes, trigger entry points, external impacts and related tests are propagated into the **six reverse index tables** of `00-Source Index.md` so that the source index and the BL entries stay consistent; when BL entries are added or retired, the knowledge map's "ID Registry" is updated in the same run (a deleted ID is a permanent tombstone that is never reused, and gaps are allowed).
