...
Mandatory provisional-BL recovery (v1.4.2): once a BL's freshness status is `Provisional Working-Tree Analysis` and the working tree becomes `Clean` again, that entry **must unconditionally join the current affected set and be re-verified against the actual source** (it does not depend on whether `<base>..HEAD` produces a diff — after `git restore` there is no diff, so the normal impact analysis cannot detect it); until every such entry has been re-verified nothing may be marked `Current`, the knowledge layer must not be treated as consistent again, and the formal baseline must not be advanced. The `Git commit` field of BL section 15 takes one of three values: `<commit hash>` / `not established (Git exists, but no formal baseline has been established yet)` / `Not Found (no Git baseline)` — the last two must be kept distinct and never conflated.
