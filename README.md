...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-v1.4.3-blue.svg" alt="Version: v1.4.3">
...
- **v1.4.3**：**规则修正**，不新增功能、不升级架构。① **修正 Provisional BL 强制恢复规则里的状态维度混用**：`部分验证` / `推断` 属于**证据状态**，`最新` / `⚠ 可能过期` 属于**时效状态**，两个维度**独立判定、严禁合并书写**；重新验证后**必须分别更新两个字段** —— 证据状态按本次实际读到的实现程度填 `已验证` / `部分验证` / `推断` / `未发现`；时效状态只有当该条针对**当前 Clean 源码**的受影响结论**全部完成核对**时才可记 `最新`，重新验证不完整、或仍存在未解决的影响项时**保持 `⚠ 可能过期`**。② **收紧正式基线推进条件**：**只有同时满足以下三条，才允许推进正式基线** —— ① 工作区 `Clean`；② **所有 affected BL 已处理完成**；③ **不存在仍为 `临时工作区分析` 或 `⚠ 可能过期` 的 affected BL**。
- **v1.4.2**：**规则收尾（最终小修）**，不新增功能、不新增文档种类。① 新增 **Provisional BL 强制恢复规则**，修复 **Dirty → Clean 后临时 BL 可能污染正式知识**的问题：`affected BL` 取**并集** —— 影响分析（Git 变更检测 + Source Map + 多层判定）命中的 BL ∪ 所有时效状态为 `临时工作区分析` 的 BL（当前工作区为 `Clean` 时）；这些条目**必须无条件加入本轮 affected set 并重新读取当前源码验证**，**不依赖 `<baseline>..HEAD` 是否存在 diff**（`git restore` 丢弃未提交修改后 diff 为空，常规影响分析发现不了）；在它们**全部重新验证完成前**：**不得标记 `最新`**、**不得认为知识层已经恢复一致**、**不得推进正式基线**。② BL 第 15 节在**正式基线未建立**时的 commit 字段取值明确为**三选一**（`<commit 哈希>` / `未建立（有 Git，但尚未建立正式基线）` / `未发现（无 Git 基线）`），其中 `未建立（有 Git，但尚未建立正式基线）` 与 `未发现（无 Git 基线）` **必须继续区分、不得混用**。
- **v1.4.1**：**规则一致性收尾**，不新增功能、不新增文档种类。① 增量维护前新增**「运行前置判定」三分支（互斥）**：**分支 A** 无 Git（或 HEAD 无效）走无 Git 降级、时效状态记 `未发现（无 Git 基线）`；**分支 B** 有 Git 但**正式基线尚未建立**（`last_verified_commit` / `正式基线` = `未建立`）时**不执行任何 `<base>..HEAD` 形式的增量 diff**（此时没有合法 base），改做**全量复核**——工作区 **Clean** 才在复核后建立正式基线，**Dirty** 则保持 `last_verified_commit` / `正式基线` = `未建立`、记 `临时工作区分析`、**不标记 `最新`**；**分支 C** 已有正式基线才做正常增量同步。**严禁**把分支 B 写成 `未发现（无 Git 基线）`（Git 实际存在，两者必须区分）。② **`traditional_docs_status` 由两值扩为三值**（`current` / `outdated` / `provisional`），并新增字段 **`traditional_docs_working_tree`**（`Clean` / `Dirty` / `未发现（无 Git）`）：Clean 生成 → `current` + `Clean`，Dirty 生成 → `provisional` + `Dirty`；三字段统一记录在 `00-项目知识地图.md`（**文档状态中心**），01–07 不增加任何独立 stale 字段，同步模式**不去修改**它们。③ 01–07 模板的版本号改为**动态占位符**，生成时按版本号规则替换，不再硬编码固定版本号。④ 时效状态判定表述统一为「**时效状态以 Git 变更检测为基础，并结合 Source Map 与多层影响分析判定**」——不再声称是纯 Git 机械判定；「工作区 `Clean` / `Dirty`」「commit baseline」「变更文件集」仍明确属于 **Git 机械结果**。⑤ 新增**真实项目验收建议**（六项测试 A–F），标准见 `SKILL.md`。
...
> **真实项目验收 / Acceptance**：本项目已进入**真实项目测试阶段**，验收标准（测试 A–F 共六项）见 [SKILL.md](SKILL.md) 的「真实项目验收建议」章节；本 README 不重复这些测试项。
...
| ⏰ 文档过期检测 / Document Staleness Detection | **时效状态以 Git 变更检测为基础，并结合 Source Map 与多层影响分析判定**：Git 负责变更检测（HEAD、baseline、变更文件集、工作区 `Clean`/`Dirty`），影响分析负责符号/API/数据/配置/调用链影响。源码证据文件在基线之后被改动（**含未提交修改**）且未重新验证时，标记 `⚠ 可能过期`；工作区 `Dirty` 时本次分析记 **`临时工作区分析`** 且**不推进正式基线**；工作区恢复 `Clean` 后，所有 `临时工作区分析` 条目**必须无条件重新纳入 affected set 并重读源码验证**（即使 `<baseline>..HEAD` 无 diff），在它们全部重验完成前**不得标记 `最新`**、**不得推进正式基线** / **Freshness status is determined from Git change detection together with the source map and multi-layer impact analysis**: Git performs change detection (HEAD, baseline, changed file set, `Clean`/`Dirty` working tree) while impact analysis covers symbols, APIs, data, configuration and call chains. Entries are marked `⚠ possibly stale` when their evidence files changed after the baseline — **including uncommitted changes** — without re-verification; while the working tree is `Dirty` the run is recorded as **`Provisional Working-Tree Analysis`** and the formal baseline is not advanced; once the working tree is `Clean` again every `Provisional Working-Tree Analysis` entry **must unconditionally re-enter the affected set and be re-verified against the source** (even when `<baseline>..HEAD` shows no diff), and until all of them are re-verified nothing may be marked `Current` and the formal baseline must not be advanced |
