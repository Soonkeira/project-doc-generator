...
**输出示例：**
```
a1b2c3d|2026-05-09|王五|添加用户登录功能
...
**用途**：作为作者获取四级优先级中的**第 3 优先级**（① 用户明确提供的作者 → ② 本地默认作者 `Soonkeira` → ③ Git 提交作者 → ④ 置空）。当用户未提供作者时，从目标项目的 Git 记录中提取作者名作为文档作者。**严禁把机器账户名（`$env:USERNAME` / `whoami`）当作作者。** 优先级规则以 `SKILL.md` 的「作者信息获取」章节为唯一权威。
...
**获取工作区变更文件（含已暂存、未暂存、未跟踪，供知识层增量检测使用）：**
```bash
git status --porcelain
```

说明：知识层的时效判定必须同时覆盖**已提交与未提交**变更，只比较 commit 区间会漏掉工作区改动，从而漏判过期。

**获取带重命名检测的文件变更列表（提交区间）：**
```bash
git diff --name-status -M <base>..HEAD
```

说明：`-M` 用于识别重命名，被重命名的证据文件需要修正路径。
...
| 版本号 | 日期 | 类型 | 模块 | 描述 | 作者 |
|--------|------|------|------|------|------|
| 1.26.509.1234 | 2026-05-09 | 新增 | 用户管理 | 添加用户登录功能 | 王五 |
