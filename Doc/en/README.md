> Version: v1.4.2 ｜ Author: [Soonkeira](https://github.com/Soonkeira)
...
5. **Mandatory recovery of provisional entries (required while the working tree is `Clean`)**: the affected set (affected BL) must be the **union**, not merely the impact-analysis hits:
   ...
   - After re-verification: content consistent with the current `Clean` source → `Current`; when it cannot be confirmed → record `Partially Verified` / `Inferred` / `⚠ Possibly Stale` according to the actual situation; `00-Source Index.md` is refreshed in the same run.
