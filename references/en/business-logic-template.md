# Project Knowledge Layer Templates (v1.4.3)
...
| Formal baseline | `<commit hash>` (advanced after re-verification only when the working tree is `Clean`, every affected BL has been processed, and no affected BL is still `Provisional Working-Tree Analysis` / `⚠ Possibly Stale`; kept at the previous formal baseline, not advanced, while `Dirty`) |
...
- **The verification baseline advances only while the working tree is clean**: run `git status --porcelain` first to determine the `Working tree status` (empty output = `Clean`, any output = `Dirty`). While `Clean`, **the formal baseline may be advanced only when all three conditions hold** (① the working tree is `Clean`; ② every affected BL has been processed; ③ no affected BL is still `Provisional Working-Tree Analysis` or `⚠ Possibly Stale`); only then may `last_verified_commit` and the `Formal baseline` be advanced, and entries whose affected conclusions have all been checked may be marked `Current`; while `Dirty`, you **must not advance `last_verified_commit`** and **must not mark anything `Current`**, and the `Verification status` is `Provisional working-tree analysis`.
...
- **After re-verification the two fields are updated separately**: the evidence status is judged by how much of the implementation was actually read in this run, and the freshness status by whether every affected conclusion has been fully checked; **neither may be substituted for the other** (see 4.2 step 4 and 4.5).
...
- **Advance the baseline only while the working tree is clean**, determined with `git status --porcelain`:
  - Working tree `Clean` (no output): **the formal baseline may be advanced only when all three conditions hold**: ① the working tree is `Clean`; ② **every affected BL has been processed**; ③ **no affected BL is still `Provisional Working-Tree Analysis` or `⚠ Possibly Stale`**. Only when those three conditions hold may `last_verified_commit` and the `Formal baseline` be advanced and the re-verified entries be marked `Current`.
...
- **Never advance the formal baseline without meeting all three advancement conditions**: advancing the formal baseline requires that all three hold at once — "the working tree is `Clean`", "every affected BL has been processed", and "no affected BL is still `Provisional Working-Tree Analysis` or `⚠ Possibly Stale`" (see 4.5).
- **Never substitute the evidence status and the freshness status for each other or write them merged**: the two must be judged separately — the evidence status by how much of the implementation was actually read in this run, and the freshness status by whether every affected conclusion has been fully checked (see 4.3 and 4.5).
